...
var sum = function(array) {
  ...
  let newSum = array.slice();
  let result = newSum.pop();
  return result + sum(newSum);
};
...
var arraySum = function(array) {
  ...
  if (Array.isArray(result)) {
    return arraySum(result) + arraySum(newSum);
  }
  return result + arraySum(newSum);
};
...
var palindrome = function(string) {
  let cleanInput = string.toLowerCase();
  cleanInput = cleanInput.split('');
  if (cleanInput.length === 0 || cleanInput.length === 1) {
    return true;
  }
  if (cleanInput.shift() !== cleanInput.pop()) {
    return false;
  }
  return palindrome(cleanInput.join(''));
};
...
var createArray = function(str) {
  ...
  resultArray.push(firstChar);
  let newString = str.slice(1);
  return resultArray.concat(createArray(newString));
};
